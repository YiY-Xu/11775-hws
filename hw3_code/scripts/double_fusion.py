...
grid = {'C': [1000], 'gamma':[0.000001], 'kernel':['rbf']}
...
if __name__ == '__main__':
    ...
    feature_types = my_params.get('l_feature_types').split(',')
    dims = my_params.get('l_feature_dims').split(',')
    feature_dims = [int(x) for x in dims]
    feature_files = ['../../features/' + x for x in my_params.get('l_feature_files').split(',')]
    ...
    SVC_model.test(False)
    SVC_model.persist_test_result('DF')
